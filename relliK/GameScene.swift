...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var score: Int = 0 {
        willSet {
            ...
            scoreBoardLabel.run(SKAction.sequence([
    ...
    var killed: Int = 0 {
        willSet {
            ...
            killBoardLabel.run(SKAction.sequence([
                SKAction.scale(to: 1.5, duration: 0.1),
                SKAction.scale(to: 1, duration: 0.1),
            ]))
        }
    ...
    var errors: Int {
        ...
        set {
            GlobalRellikConcurrent.sync {
                ...
                errorsBoardLabel.run(SKAction.sequence([
                    SKAction.scale(to: 1.5, duration: 0.1),
                    SKAction.scale(to: 1, duration: 0.1),
                ]))
            }
    ...
    var highscores: Int = 0 {
        willSet {
            ...
            highScoreBoardLabel.run(SKAction.sequence([
                SKAction.scale(to: 1.5, duration: 0.1),
                SKAction.scale(to: 1, duration: 0.1),
            ]))
        }
    ...
    var gameTimer: Int = 0 {
        willSet {
            ...
            timerBoardLabel.run(SKAction.sequence([
                SKAction.scale(to: 1.5, duration: 0.1),
                SKAction.scale(to: 1, duration: 0.1),
            ]))
        }
    ...
    override func update(_ currentTime: TimeInterval) {
        if !isGamePaused && gameState == .playing {
          kdebug_signpost_start(20, 0, 0, 0, 0)
          ...
          kdebug_signpost_end(20, 0, 0, 0, 0)
        }
    }
    ...
    func setup() {
      kdebug_signpost_start(25, 0, 0, 0, 0)
        ...
        group.notify(queue: GlobalRellikSerial, execute: {
          ...
          kdebug_signpost_end(25, 0, 0, 0, 0)
        })
    }
...
extension GameScene {
    ...
    func moveEnemies() {
      kdebug_signpost_start(15, 0, 0, 0, 0)
        //    let group = DispatchGroup()
        ...
        //    }
      kdebug_signpost_end(15, 0, 0, 0, 0)
    }
    ...
    func randomEnemy(_ enemyLocation: CGPoint, delegate _: SceneUpdateProtocol) -> Enemy {
    ...
    func shot(enemyDirection: entityDirection, num: Int) -> [() -> Void] {
        var blocks: [() -> Void] = []
        //    GlobalRellikConcurrent.async {
